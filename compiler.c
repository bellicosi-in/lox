...
//to deal with the looping part of the code
...
//backpatching to put in the number of bytes to offset if the condition is false. 
...
//to deal with the logical operator AND 
...
//to deal with the logical operator OR
static void or_(bool canAssign){
    //jumps to OP_POP if the left expression is false.
    int elseJump = emitJump(OP_JUMP_IF_FALSE);

    //jumps to the end of the oR expression if the left expression is true. 
...
static void classDeclaration(){
    ...
    if(match(TOKEN_LESS)){
        consume(TOKEN_LESS, "Expect superclass name");
        variable(false);
...
//dealing with the for statement
static void forStatement(){
    beginScope();
    consume(TOKEN_LEFT_PAREN,"Expect '(' after the for");
    ...
    if(!match(TOKEN_RIGHT_PAREN)){
        //unconditional jump to skip the increment part of the body
...
static void whileStatement(){
    ...
    expression();
    consume(TOKEN_LEFT_PAREN,"expect ')' after the condition");
